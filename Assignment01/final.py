...
def train_model(model, X_train, y_train, X_test, y_test):
    ...
    # Removed ROC AUC computation as it's no longer needed
    
    # Return evaluation metrics
    return model, accuracy, f1

# Train models and gather F1 scores
model_lr = LogisticRegression()
time_start = time.time()
model_lr, acc_lr, f1_lr = train_model(model_lr, x_train, y_train, x_test, y_test)
time_taken_lr = time.time() - time_start

model_gnb = GaussianNB()
time_start = time.time()
model_gnb, acc_gnb, f1_gnb = train_model(model_gnb, x_train, y_train, x_test, y_test)
time_taken_gnb = time.time() - time_start

model_dt = DecisionTreeClassifier()
time_start = time.time()
model_dt, acc_dt, f1_dt = train_model(model_dt, x_train, y_train, x_test, y_test)
time_taken_dt = time.time() - time_start

model_rf = RandomForestClassifier()
time_start = time.time()
model_rf, acc_rf, f1_rf = train_model(model_rf, x_train, y_train, x_test, y_test)
time_taken_rf = time.time() - time_start

model_xgb = xgb.XGBRFClassifier()
time_start = time.time()
model_xgb, acc_xgb, f1_xgb = train_model(model_xgb, x_train, y_train, x_test, y_test)
time_taken_xgb = time.time() - time_start

# Update the final DataFrame to focus on F1 Scores
accuracies = [acc_lr, acc_gnb, acc_dt, acc_rf, acc_xgb]
f_scores = [f1_lr, f1_gnb, f1_dt, f1_rf, f1_xgb]
time = [time_taken_lr, time_taken_gnb, time_taken_dt, time_taken_rf, time_taken_xgb]

final_df = pd.DataFrame({"Accuracies": accuracies, "F1 Scores": f_scores, "Time Taken": time}, 
                       index=['LogisticReg', 'GaussianNB', 'DecisionTree', 'RandomForest', 'XGB'])
final_df = final_df.round(4)

# Finding the model with the maximum F1 Score
max_f1_index = final_df['F1 Scores'].idxmax()
best_model_name = max_f1_index
best_model_f1_score = final_df.loc[max_f1_index, 'F1 Scores']

# Printing the best model name and its F1 score
print(f"BestModel:{best_model_name}")
print(f"F1:{best_model_f1_score}")